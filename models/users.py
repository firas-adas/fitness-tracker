...
class User(db.Model):
    ...
    subscription_status = db.Column(db.String(20), nullable=True, default='Active')
    last_update = db.Column(db.DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

def get_users():
    return User.query.all()

def add_user(first_name, last_name, email, birthdate=None, gender=None, activity_level=None, experience=None):
